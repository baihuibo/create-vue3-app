...
export default function Schedule(opt) {
    var def = {},
    ...
    var init = function () {
        ...
        var todayBtn = '<div class="row"> ' +
            '<span id="todayBtn" class="today">今天</span>' +
            '<span id="clearBtn" class="today">清空</span>' +
            '</div>'
    ...
    this.renderToday = function () {
        if (opt.clickCb) {
            const now = new Date();
            opt.clickCb(concatDate(now.getFullYear(), now.getMonth() + 1, now.getDate()))
        }
    }
